	...
	: Super(ObjectInitializer)
{
	...
	bDisableGravityReplication = true;
...
FVector UCustomCharacterMovementComponent::GetImpartedMovementBaseVelocity() const
{
...
void UCustomCharacterMovementComponent::JumpOff(AActor* MovementBaseActor)
{
...
FVector UCustomCharacterMovementComponent::GetBestDirectionOffActor(AActor* BaseActor) const
{
...
void UCustomCharacterMovementComponent::SetDefaultMovementMode()
{
...
void UCustomCharacterMovementComponent::PerformAirControlForPathFollowing(FVector Direction, float ZDiff)
{
...
void UCustomCharacterMovementComponent::PerformAirControlForPathFollowingEx(const FVector& MoveVelocity, const FVector& GravDir)
{
	...
	if (CharacterOwner && (Velocity | GravDir) > 0.0f && (MoveSpeedZ < 0.0f || GetGravityMagnitude() < FMath::Abs(0.9f * GetWorld()->GetDefaultGravityZ())))
	{
		if (MoveSpeedZ > 0.0f)
		{
			if (MoveSpeedZ > 2.0f * GetMaxJumpHeight() && PathFollowingComp.IsValid())
			{
...
FVector UCustomCharacterMovementComponent::CalcRootMotionVelocity(const FVector& RootMotionDeltaMove, float DeltaSeconds, const FVector& CurrentVelocity) const
{
	if (ensure(DeltaSeconds > 0.0f))
	{
		...
		if (IsFalling())
		{
			...
			RootMotionVelocity = FVector::VectorPlaneProject(RootMotionVelocity, GravityDir) + GravityDir * (CurrentVelocity | GravityDir);
		}
...
void UCustomCharacterMovementComponent::SimulateMovement(float DeltaSeconds)
{
...
void UCustomCharacterMovementComponent::MaybeUpdateBasedMovement(float DeltaSeconds)
{
...
void UCustomCharacterMovementComponent::UpdateBasedRotation(FRotator& FinalRotation, const FRotator& ReducedRotation)
{
...
float UCustomCharacterMovementComponent::SlideAlongSurface(const FVector& Delta, float Time, const FVector& InNormal, FHitResult& Hit, bool bHandleImpact)
{
	if (!Hit.bBlockingHit)
	{
		return 0.0f;
...
void UCustomCharacterMovementComponent::TwoWallAdjust(FVector& Delta, const FHitResult& Hit, const FVector& OldHitNormal) const
{
...
FVector UCustomCharacterMovementComponent::HandleSlopeBoosting(const FVector& SlideResult, const FVector& Delta, const float Time, const FVector& Normal, const FHitResult& Hit) const
{
	const FVector CapsuleUp = GetComponentAxisZ();
...
float UCustomCharacterMovementComponent::ImmersionDepth() const
{
	float Depth = 0.0f;
...
void UCustomCharacterMovementComponent::RequestDirectMove(const FVector& MoveVelocity, bool bForceMaxSpeed)
{
...
float UCustomCharacterMovementComponent::GetMaxJumpHeight() const
{
...
void UCustomCharacterMovementComponent::PhysFlying(float deltaTime, int32 Iterations)
{
...
void UCustomCharacterMovementComponent::ApplyRootMotionToVelocityOVERRIDEN(float deltaTime)
{
...
void UCustomCharacterMovementComponent::PhysSwimming(float deltaTime, int32 Iterations)
{
	if (deltaTime < MIN_TICK_TIME)
	{
		return;
...
void UCustomCharacterMovementComponent::StartSwimmingOVERRIDEN(FVector OldLocation, FVector OldVelocity, float timeTick, float remainingTime, int32 Iterations)
{
...
FVector UCustomCharacterMovementComponent::GetFallingLateralAcceleration(float DeltaTime)
{
	return GetFallingLateralAccelerationEx(DeltaTime, GetGravityDirection(true));
}

FVector UCustomCharacterMovementComponent::GetFallingLateralAccelerationEx(float DeltaTime, const FVector& GravDir) const
{
...
FVector UCustomCharacterMovementComponent::GetAirControl(float DeltaTime, float TickAirControl, const FVector& FallAcceleration)
{
	return GetAirControlEx(DeltaTime, TickAirControl, FallAcceleration, GetGravityDirection(true));
}

FVector UCustomCharacterMovementComponent::GetAirControlEx(float DeltaTime, float TickAirControl, const FVector& FallAcceleration, const FVector& GravDir) const
{
	// Boost.
...
float UCustomCharacterMovementComponent::BoostAirControl(float DeltaTime, float TickAirControl, const FVector& FallAcceleration)
{
	return BoostAirControlEx(DeltaTime, TickAirControl, FallAcceleration, GetGravityDirection(true));
}

float UCustomCharacterMovementComponent::BoostAirControlEx(float DeltaTime, float TickAirControl, const FVector& FallAcceleration, const FVector& GravDir) const
{
...
FVector UCustomCharacterMovementComponent::LimitAirControl(float DeltaTime, const FVector& FallAcceleration, const FHitResult& HitResult, bool bCheckForValidLandingSpot)
{
	return LimitAirControlEx(DeltaTime, FallAcceleration, HitResult, GetGravityDirection(true), bCheckForValidLandingSpot);
}

FVector UCustomCharacterMovementComponent::LimitAirControlEx(float DeltaTime, const FVector& FallAcceleration, const FHitResult& HitResult, const FVector& GravDir, bool bCheckForValidLandingSpot) const
{
...
bool UCustomCharacterMovementComponent::CheckLedgeDirection(const FVector& OldLocation, const FVector& SideStep, const FVector& GravDir) const
{
...
FVector UCustomCharacterMovementComponent::GetLedgeMove(const FVector& OldLocation, const FVector& Delta, const FVector& GravDir) const
{
...
void UCustomCharacterMovementComponent::StartFalling(int32 Iterations, float remainingTime, float timeTick, const FVector& Delta, const FVector& subLoc)
{
...
FVector UCustomCharacterMovementComponent::ComputeGroundMovementDelta(const FVector& Delta, const FHitResult& RampHit, const bool bHitFromLineTrace) const
{
	const FVector CapsuleUp = GetComponentAxisZ();
	return ComputeGroundMovementDeltaEx(FVector::VectorPlaneProject(Delta, CapsuleUp), CapsuleUp, RampHit, bHitFromLineTrace);
}

FVector UCustomCharacterMovementComponent::ComputeGroundMovementDeltaEx(const FVector& Delta, const FVector& DeltaPlaneNormal, const FHitResult& RampHit, const bool bHitFromLineTrace) const
{
...
void UCustomCharacterMovementComponent::MoveAlongFloor(const FVector& InVelocity, float DeltaSeconds, FStepDownResult* OutStepDownResult)
{
	if (!CurrentFloor.IsWalkableFloor())
	{
		return;
...
void UCustomCharacterMovementComponent::MaintainHorizontalGroundVelocity()
{
	if (bMaintainHorizontalGroundVelocity)
	{
...
void UCustomCharacterMovementComponent::AdjustFloorHeight()
{
...
void UCustomCharacterMovementComponent::SetPostLandedPhysics(const FHitResult& Hit)
{
	if (CharacterOwner)
	{
		if (CanEverSwim() && IsInWater())
...
void UCustomCharacterMovementComponent::OnTeleported()
{
	if (!HasValidData())
	{
		return;
...
void UCustomCharacterMovementComponent::PhysicsVolumeChanged(class APhysicsVolume* NewVolume)
{
	if (!HasValidData())
	{
		return;
...
bool UCustomCharacterMovementComponent::ShouldJumpOutOfWater(FVector& JumpDir)
{
	return ShouldJumpOutOfWaterEx(JumpDir, GetGravityDirection(true));
}

bool UCustomCharacterMovementComponent::ShouldJumpOutOfWaterEx(FVector& JumpDir, const FVector& GravDir)
{
...
bool UCustomCharacterMovementComponent::CheckWaterJump(FVector CheckPoint, FVector& WallNormal)
{
	return CheckWaterJumpEx(CheckPoint, GetGravityDirection(true), WallNormal);
}

bool UCustomCharacterMovementComponent::CheckWaterJumpEx(FVector CheckPoint, const FVector& GravDir, FVector& WallNormal)
{
	if (!HasValidData())
	{
		return false;
...
bool UCustomCharacterMovementComponent::IsWithinEdgeTolerance(const FVector& CapsuleLocation, const FVector& TestImpactPoint, const float CapsuleRadius) const
{
	return IsWithinEdgeToleranceEx(CapsuleLocation, GetComponentAxisZ() * -1.0f, CapsuleRadius, TestImpactPoint);
}

bool UCustomCharacterMovementComponent::IsWithinEdgeToleranceEx(const FVector& CapsuleLocation, const FVector& CapsuleDown, const float CapsuleRadius, const FVector& TestImpactPoint) const
{
...
void UCustomCharacterMovementComponent::ComputeFloorDist(const FVector& CapsuleLocation, float LineDistance, float SweepDistance, FFindFloorResult& OutFloorResult, float SweepRadius, const FHitResult* DownwardSweepResult) const
{
...
bool UCustomCharacterMovementComponent::FloorSweepTest(FHitResult& OutHit, const FVector& Start, const FVector& End, ECollisionChannel TraceChannel,
...
bool UCustomCharacterMovementComponent::IsValidLandingSpot(const FVector& CapsuleLocation, const FHitResult& Hit) const
{
	if (!Hit.bBlockingHit)
	{
		return false;
...
bool UCustomCharacterMovementComponent::ShouldCheckForValidLandingSpot(float DeltaTime, const FVector& Delta, const FHitResult& Hit) const
{
	const FVector CapsuleUp = GetComponentAxisZ();
...
bool UCustomCharacterMovementComponent::ShouldComputePerchResult(const FHitResult& InHit, bool bCheckRadius) const
{
...
bool UCustomCharacterMovementComponent::ComputePerchResult(const float TestRadius, const FHitResult& InHit, const float InMaxFloorDist, FFindFloorResult& OutPerchFloorResult) const
{
...
bool UCustomCharacterMovementComponent::StepUp(const FVector& GravDir, const FVector& Delta, const FHitResult& InHit, struct UCharacterMovementComponent::FStepDownResult* OutStepDownResult)
{
...
void UCustomCharacterMovementComponent::HandleImpact(const FHitResult& Impact, float TimeSlice, const FVector& MoveDelta)
{
	if (CharacterOwner)
	{
...
void UCustomCharacterMovementComponent::ApplyImpactPhysicsForces(const FHitResult& Impact, const FVector& ImpactAcceleration, const FVector& ImpactVelocity)
{
...
void UCustomCharacterMovementComponent::DisplayDebug(UCanvas* Canvas, const FDebugDisplayInfo& DebugDisplay, float& YL, float& YPos)
{
...
void UCustomCharacterMovementComponent::VisualizeMovement() const
{
...
FVector UCustomCharacterMovementComponent::ConstrainInputAcceleration(const FVector& InputAcceleration) const
{
...
void UCustomCharacterMovementComponent::ServerMoveHandleClientError(float ClientTimeStamp, float DeltaTime, const FVector& Accel, const FVector& RelativeClientLoc, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode)
{
...
//void UCustomCharacterMovementComponent::ClientAdjustPosition_Implementation(float TimeStamp, FVector NewLocation, FVector NewVelocity, UPrimitiveComponent* NewBase, FName NewBaseBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode)
//{
...
void UCustomCharacterMovementComponent::CapsuleTouched(UPrimitiveComponent* OverlappedComp, AActor* Other, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	if (!bEnablePhysicsInteraction)
	{
		return;
...
void UCustomCharacterMovementComponent::ApplyDownwardForce(float DeltaSeconds)
{
...
void UCustomCharacterMovementComponent::ApplyRepulsionForce(float DeltaSeconds)
{
...
void UCustomCharacterMovementComponent::ApplyAccumulatedForces(float DeltaSeconds)
{
...
FVector UCustomCharacterMovementComponent::GetGravity() const
{
...
FVector UCustomCharacterMovementComponent::GetGravityDirection(bool bAvoidZeroGravity) const
{
...
float UCustomCharacterMovementComponent::GetGravityMagnitude() const
{
...
FORCEINLINE void UCustomCharacterMovementComponent::SetCustomGravityDirection(const FVector& NewCustomGravityDirection)
{
...
void UCustomCharacterMovementComponent::ClientSetCustomGravityDirection_Implementation(const FVector& NewCustomGravityDirection)
{
	SetCustomGravityDirection(NewCustomGravityDirection);
}

void UCustomCharacterMovementComponent::ClientClearCustomGravityDirection_Implementation()
{
	SetCustomGravityDirection(FVector::ZeroVector);
}

void UCustomCharacterMovementComponent::ClientSetGravityPoint_Implementation(const FVector& NewGravityPoint)
{
	GravityPoint = NewGravityPoint;
}

void UCustomCharacterMovementComponent::ClientClearGravityPoint_Implementation()
{
	GravityPoint = FVector::ZeroVector;
}

void UCustomCharacterMovementComponent::ClientSetGravityScale_Implementation(float NewGravityScale)
{
	GravityScale = NewGravityScale;
}

void UCustomCharacterMovementComponent::UpdateGravity(float DeltaTime)
{
...
FRotator UCustomCharacterMovementComponent::ConstrainComponentRotation(const FRotator& Rotation) const
{
...
FORCEINLINE FVector UCustomCharacterMovementComponent::GetComponentAxisX() const
{
...
FORCEINLINE FVector UCustomCharacterMovementComponent::GetComponentAxisZ() const
{
...
FVector UCustomCharacterMovementComponent::GetComponentDesiredAxisZ() const
{
...
void UCustomCharacterMovementComponent::UpdateComponentRotation()
{
	if (!HasValidData())
	{
		return;
...
void UCustomCharacterMovementComponent::ClientAdjustPosition_Implementation(float TimeStamp, FVector NewLocation, FVector NewVelocity, UPrimitiveComponent* NewBase, FName NewBaseBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode)
{
	...
	AMainCharacter* UTOwner = Cast<AMainCharacter>(CharacterOwner);
	if (UTOwner == NULL || UTOwner->GetRootComponent() == NULL || (!UTOwner->GetRootComponent()->IsSimulatingPhysics() && !UTOwner->GetMesh()->IsSimulatingPhysics()))
	{
		bool bWasFalling = (MovementMode == MOVE_Falling);
...
void UCustomCharacterMovementComponent::ClientAckGoodMove_Implementation(float TimeStamp)
{
	if (!HasValidData())
	{
		return;
	...
	//AConceptPlayerState* UTPS = Cast<AConceptPlayerState>(CharacterOwner->PlayerState);
	//if (UTPS)
	//{
...
void UCustomCharacterMovementComponent::SendClientAdjustment()
{
	if (!HasValidData())
	{
		return;
...
void UCustomCharacterMovementComponent::ClientNoBaseAdjustPosition_Implementation(float TimeStamp, FVector NewLocation, FVector NewVelocity, uint8 ServerMovementMode)
{
	if (!HasValidData() || !IsComponentTickEnabled())
	{
		return;
